...
батник build.bat собирает питоном .exe файл, для выполнения на любом пк, пакуя его всеми необходимыми пакетами для работы из коробки. правда не стал туда же подтягивать поддержку CUDA для gpu local llm ибо бесплатные лимиты groq вполне меня устраивают, так же как и скорость!, поэтому регистрируйтесь, создавайте ключ, копипаст и вперед! exe если что будет весить 230 +- mb 
